...
if uploaded_file is not None:
    try:
        ...
        # ────────────────────────────────────────
        # GRID ENERGY INPUT → this should now be visible right after metrics
        # ────────────────────────────────────────
        ...
        # ────────────────────────────────────────
        # KEY METRICS (updated: replaced battery charge with total consumption)
        # ────────────────────────────────────────
        ...
        with tab1:
            ...
            fig1 = go.Figure()
            fig1.add_trace(go.Scatter(x=df_filtered["Date"], y=df_filtered.get("Theoretical_Yield", pd.Series(0)),
                                      name="Үйлдвэрлэх боломжит [кВт.ц]", line=dict(color='#10b981', width=2.5)))
            fig1.add_trace(go.Scatter(x=df_filtered["Date"], y=df_filtered.get("Inverter_Yield", pd.Series(0)),
        ...
        with tab2:
            ...
            fig2 = px.bar(df_filtered, x="Date",
